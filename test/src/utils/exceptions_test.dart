  ...
  group('API Exceptions', () {
    group('ApiException', () {
      ...
      test('should handle null status code', () {
        ...
        expect(exception.toString(), equals('ApiException: Error without status code'));
      });
    ...
    group('BadRequestException', () {
      test('should have correct status code and message format', () {
        ...
        expect(exception.toString(), equals('BadRequestException: Invalid request parameters'));
      });
      ...
      test('should handle detailed error messages', () {
        // Arrange & Act
        final exception = BadRequestException('{"field": "email", "error": "Invalid format"}');
        
        // Assert
        expect(exception.message, equals('{"field": "email", "error": "Invalid format"}'));
        expect(exception.statusCode, equals(400));
      });
    });
    ...
    group('UnauthorizedException', () {
      test('should have correct status code and message format', () {
        ...
        expect(exception.toString(), equals('UnauthorizedException: Authentication failed'));
      });
    ...
    group('ForbiddenException', () {
      test('should have correct status code and message format', () {
        ...
        expect(exception.toString(), equals('ForbiddenException: Insufficient permissions'));
      });
    ...
    group('NotFoundException', () {
      test('should have correct status code and message format', () {
        ...
        expect(exception.toString(), equals('NotFoundException: Resource not found'));
      });
    ...
    group('RateLimitExceededException', () {
      test('should have correct status code and message format', () {
        ...
        expect(exception.toString(), equals('RateLimitExceededException: Too many requests'));
      });
      ...
      test('should handle message with retry information', () {
        // Arrange & Act
        final exception = RateLimitExceededException('Rate limit exceeded. Try again in 60 seconds.');
        
        // Assert
        expect(exception.message, equals('Rate limit exceeded. Try again in 60 seconds.'));
        expect(exception.statusCode, equals(429));
      });
    });
    ...
    test('exceptions with same message should have different string representations', () {
      // Arrange
      const testMessage = 'Test error message';
      ...
      expect(badRequest.toString(), equals('BadRequestException: $testMessage'));
      expect(unauthorized.toString(), equals('UnauthorizedException: $testMessage'));
      expect(apiException.toString() != badRequest.toString(), isTrue);
      expect(badRequest.toString() != unauthorized.toString(), isTrue);
    });
    
    test('should handle different exception types with different status codes', () {
      // Arrange & Act
      final exceptions = [
    ...
    test('should be able to throw and catch different exception types', () {
      // Act & Assert
      expect(() => throw ApiException('Generic error'), throwsA(isA<ApiException>()));
      expect(() => throw BadRequestException('Bad request'), throwsA(isA<BadRequestException>()));
      expect(() => throw UnauthorizedException('Unauthorized'), throwsA(isA<UnauthorizedException>()));
      expect(() => throw ForbiddenException('Forbidden'), throwsA(isA<ForbiddenException>()));
      expect(() => throw NotFoundException('Not found'), throwsA(isA<NotFoundException>()));
      expect(() => throw RateLimitExceededException('Rate limit'), throwsA(isA<RateLimitExceededException>()));
    });
  });
}